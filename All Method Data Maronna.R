...
temp<-matrix(rnorm(p)) ; b=temp/norm(temp,"F")
...
# %%%%%%%%%%%%%%%%%%%%%%   DATA PREPARATION    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% #
# reading the full dataset; table of dimension 3220x9995
QTLMAS_all <- read.delim(file="~/Documents/Cours/M2 - Math En Action/Stage - Robust/TP Ogutu/QTLMAS2011_all.txt")

# the training dataset includes only observations with observed value of yield
# i.e., the data consists of vectors Entry sire dam cross yield SNP1...SNP9990
# i.e., table of 2000 x 9995
training <- QTLMAS_all[which(QTLMAS_all$yield != "NA"),]

# the remaining data will be used for prediction; table of 1220 x 9995
# i.e., later we will predict 1220 genotypes
Newdata <- QTLMAS_all[which(is.na(QTLMAS_all$yield)),]
entry.names<-Newdata[,1]

# separating the training dataset in yield and the SNPs alone
tmp_x <- training[,substr(names(training),1,3)=="COL"]
x     <- as.matrix(tmp_x)
y_train     <- training[,"yield"]

# separating the dataset for prediction in the SNP data alone
val_x <- Newdata[,substr(names(Newdata),1,3)=="COL"]
val_newx <-as.matrix (val_x)
rownames(val_newx)<-Newdata[,1]

# cleaning up the SNP marker matrix for SNPs with var=0 in both the training and validation SNP datasets
X_train<-x[,which(apply(x,2,var)!=0)] # exclude markers with variance =0
nonzero   <-intersect(colnames(X_train),colnames(val_newx))  
val_newx2 <-val_newx[,nonzero]

# standardizing and centering the SNPs for prediction  
meanx_2 <-apply(val_newx2,2,mean)                 #mean of each marker
X_test <-scale (val_newx2,meanx_2, FALSE)   #standadized markers

# loading the TBVs; we only have 1000 of these  ~/Documents/M2 - Math En Action/Stage - Robust/TP Ogutu$ 


TBVs<-read.delim(file="~/Documents/Cours/M2 - Math En Action/Stage - Robust/TP Ogutu/TBVsValidationSet2.txt")
...
#####################################################
#source("~/Documents/Cours/M2 - Math En Action/Robust-Stats-main/Berhu Penalization/Launch_Function.R")
#ptm<-proc.time()
...
#             fold.id=foldid,nfolds = nb_folds,ncore=10,type.measure="deviance")
...
#                        ncore=ncore,measure="robust mse",plot.path = F,bestlambda = "1se",plot.cv = F)$beta
...
cv<-cv_Berhu(X_train,y_train,pena="Elastic Net",loss="Huber",M=1.345,alpha=1,
             ...
             ncore=ncore,type.measure="deviance")
...
source("~/Documents/Cours/M2 - Math En Action/Robust-Stats-main/Berhu Penalization/Launch_Function.R")
ptm<-proc.time()
cv<-cv_Berhu(X_train,y_train,pena="Berhu",loss="Huber",M=1.345,alpha=1,
             FUN="Berhu",nlambda=100,
             fold.id=foldid,nfolds = nb_folds,
             ncore=ncore,type.measure="deviance")
...
intercept=cv$mu.min
predict.RobBerhu=X_test%*%beta.RobLassoScs+intercept
...
predictions <- cbind(predict.EN,predict.Lasso,predict.Ridge,predict.RidgeBlup,
                    predict.RobRidgeHqreg,#predict.RobRidgeScs,
                    predict.RobRidgeMaronna,
...
time<-cbind(time.EN,time.Lasso,time.Ridge,time.RidgeBlup,
            time.RobRidgeHqreg,#time.RobRidgeScs,
            time.RobRidgeMaronna,
