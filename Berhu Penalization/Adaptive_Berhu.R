...
Adaptive_Berhu<-function(X,y,M=1.345,L=1.345,lambda,
                                  beta_ini="unpenalized", loss=c("Huber","Least Square"),
                                  nlambda=10,lambda_min=0.003,lambda_max){
  ...
  #Loss
  Huber_Owen<-function(beta,v,mu, sigma=1){(quad_over_lin(y-X%*%beta-v-mu,sigma)+2*M*p_norm(v,1)+n*sigma)/(2*n)}
